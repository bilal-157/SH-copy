...
namespace jewl
{
    public partial class GoldSale : Form
    {
        ...
        private void GoldSale_Load(object sender, EventArgs e)
        {
            ...
            cbxSaleMan.Select();
            this.txtGSaleNo.Text = (pgDAL.GetMaxGSNO() + 1).ToString();
        }
        ...
        private void dgvGoldPurchaseDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            cstmr = custDAL.SearchCustById(gld.CustId);
            this.txtCustomerName.Text = cstmr.Name.ToString();
            this.cbxContactNo.Text = cstmr.Mobile.ToString();
            ...
            this.lblCustId.Text = gld.CustId.ToString();
            chld = new AccountDAL().GetAccount(1, "Current Asset", "Cash In Hand");
            if (this.lblCPV.Text != "")
            {
                this.txtCashReceive.Text = Math.Round(vcDAL.GetCash(this.lblCPV.Text, chld.ChildCode), 0).ToString();
            }
            if (this.lblCPV.Text == "")
                this.txtCashReceive.Text = "0";
            this.lblBalance.Text = (FormControls.GetDecimalValue(this.txtCustomerBalace, 0) + FormControls.GetDecimalValue(this.txtCashReceive, 0) - FormControls.GetDecimalValue(this.txtPrice, 0)).ToString();
            this.lblCashBalance.Text = (FormControls.GetDecimalValue(this.lblCashHand, 0) - FormControls.GetDecimalValue(this.txtCashReceive, 0)).ToString();
        ...
        private void txtWeight_TextChanged(object sender, EventArgs e)
        {
            if (this.btnEdit.Text == "Update")
            {
                if (this.txtWeight.Text != "")
                {
                    ...
                    this.txtPrice.Text = Math.Round((Convert.ToDecimal(this.txtPureWeight.Text) * Convert.ToDecimal(this.txtRate.Text)), 0).ToString();
                }
                else
                {
                    this.txtPureWeight.Text = "";
        ...
        {            
            try
            {
                ...
                for (int i = 0; i < this.dgvDetail.Rows.Count - 1; i++)
                {
                    ...
                    pg.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
                ...
                vchr.Dr = 0;               
                vchr.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
                ...
                vchr.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
                this.vcDAL.AddVoucher(vchr, con1, trans);
                if (this.txtCashReceive.Text != "")
                {
                    vchr.AccountCode = child;
                    vchr.Cr = FormControls.GetDecimalValue(this.txtCashReceive, 0);
                    vchr.Dr = 0;
                    vchr.SNO = pg.SNO;
                    vchr.OrderNo = pg.ONO;
                    vchr.VNO = vcDAL.CreateVNO("CRV",con1 ,trans );
                    vchr.DDate = pg.PGDate;
                    vchr.Description = pg.Description;
                    vchr.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
                    this.vcDAL.AddVoucher(vchr, con1, trans);
                    ...
                    cha.Balance = cha.Balance + FormControls.GetDecimalValue(this.txtCashReceive, 0);
                    aDAL.UpdateChildBalance(cha.Balance, cha.ChildCode, con1, trans);
                    vchr.AccountCode = cha;
                    vchr.Dr = FormControls.GetDecimalValue(this.txtCashReceive, 0);
                    vchr.Cr = 0;
                    vchr.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
                    this.vcDAL.AddVoucher(vchr, con1, trans);                                      
                }             
            ...
            finally
            {
                if (con1.State == ConnectionState.Open)
                {
                    ...
                    frm.id = 1;
                    frm.selectQuery = "{GoldSalePurchaseBill.GSNO}=" + pg.GSNO;
                    frm.ShowDialog();
                    this.Dispose();
                    GoldSale frm1 = new GoldSale();
                    frm1.ShowDialog();
                }
            }
        }
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (btnEdit.Text == "Update")
            {
                try
                {
                    ...
                    if (this.cbxSaleMan.Text == "" && this.cbxSaleMan.SelectedIndex == -1)
                    {
                        MessageBox.Show("Please Select SaleManName", Messages.Header);
                        this.cbxSaleMan.Focus();
                        return;
                    }                    
                    con1.Open();
                    trans = con1.BeginTransaction();
                    pgDAL.DeleteSaleByPNo(Convert.ToInt32(this.txtGSaleNo.Text), con1, trans);
                    for (int i = 0; i < this.dgvDetail.Rows.Count - 1; i++)
                    {
                        pg.PGDate = Convert.ToDateTime(this.dtpPGDate.Value);
                        pg.GSNO = Convert.ToInt32(this.txtGSaleNo.Text);
            ...
            if (btnEdit.Text == "&Edit")
            {
                ...
                espno.ShowDialog();
                GetSaleGoldbySNo(espno.PNO);
                btnSave.Enabled = false;
                return;
            }
        }

        private void GetSaleGoldbySNo(int SNo)
        {
            PGL = pgDAL.GetSaleBySNo(SNo);
            if (PGL == null)
            {
            ...
            else
            {
                ...
                this.txtCashReceive.Text = Math.Round(PGL[0].CashPR, 0).ToString();
                this.txtGSaleNo.Text = SNo.ToString();
        ...
        private void rbtUsedGold_CheckedChanged(object sender, EventArgs e)
        {
            if (this.rbtUsedGold.Checked == true)
            {
                if (this.txtPureWeight.Text != string.Empty)
                {
                    if (FormControls.GetDecimalValue(this.txtPureWeight, 3) > (this.lblUsedGold.Text==""?0: Convert.ToDecimal(this.lblUsedGold.Text)))
                    {
                        MessageBox.Show("Used Gold Is Not Available You Entered", Messages.Header);
        ...
        {
            if (this.txtPureWeight.Text == "" || this .txtRate .Text =="")
            {
                if (this.txtRate.Text != string.Empty)
                {
                    this.txtPrice.Text = Math.Round((FormControls.GetDecimalValue(this.txtRate, 0) * 0), 0).ToString();
                    decimal a = FormControls.GetDecimalValue(this.txtPrice, 0);
                }
            }
            else
            {
        ...
        private void cbxCustomerName_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (cbxCustomerName.SelectedIndex != -1)
                {
                    ...
                    this.txtCustomerBalace.Text = cust.CashBalance.ToString();
                }
                cbxContactNo.Select();
            }
        }
            ...
            if (!(this.dgvDetail.Rows.Count <= 1))
            {
                for (counter = 0; counter < (dgvDetail.Rows.Count - 1); counter++)
                {
                    if (Convert.ToString(dgvDetail.Rows[counter].Cells[6].Value) == string.Empty || dgvDetail.Rows[counter].Cells[6].Value == null)
                        ...
                        weight += decimal.Parse(dgvDetail.Rows[counter].Cells[6].Value.ToString());
                }
            }
            return weight;
        }

        public decimal upDateRecAmountTextBox()
        {
            decimal rPrice = 0;
            int counter;
            if (!(this.dgvDetail.Rows.Count <= 1))
            {
                for (counter = 0; counter < (dgvDetail.Rows.Count - 1); counter++)
                {
                    if (Convert.ToString(dgvDetail.Rows[counter].Cells[6].Value) == string.Empty || dgvDetail.Rows[counter].Cells[6].Value == null)
                        ...
                        rPrice += decimal.Parse(dgvDetail.Rows[counter].Cells[6].Value.ToString());
                }
            }
            return rPrice;
        }
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            {
                this.txtTAmount.Text = Math.Round(upDateAmountTextBox(), 0).ToString();
                this.txtCashReceive.Text = Math.Round(upDateRecAmountTextBox(), 0).ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("Are you sure want to delete this Record  Click Yes else Click No", Messages.Header, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                con1.Open();
                trans = con1.BeginTransaction();
                pgDAL.DeleteSaleByPNo(espno.PNO, con1, trans);
                trans.Commit();
                con1.Close();
                MessageBox.Show(Messages.Deleted);
                GoldSale gp = new GoldSale();
                this.Dispose();
                gp.ShowDialog();
            }
            else
        ...
        private void txtRAmount_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
