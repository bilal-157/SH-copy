...
namespace jewl
{
    public partial class CashReceiptVoucher : Form
    {
        ...
        private Voucher pv;
        private Voucher custv; DataGridViewRow dr = new DataGridViewRow();
        string str = "", strAmount = "";
        int r;
        string st = "";
        string b = "";
        decimal h;
        ...
        private void CashReceiptVoucher_Load(object sender, EventArgs e)
        {
            ...
            this.ShowLedger();
            this.txtVoucherNo.Text = vDAL.CreateVNO("CRV");
            strAmount = "";
        ...
        public decimal updateSum()
        {
            ...
            {
                if ((string)dgvTransection.Rows[counter].Cells[2].Value == "" || (string)dgvTransection.Rows[counter].Cells[2].Value == null)
                    k = 0;
                else
                    k = Convert.ToDecimal((string)this.dgvTransection.Rows[counter].Cells[2].Value);
        ...
        private void dgvTransection_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            this.lblGoldBalance.Text = Math.Round(vDAL.GetGoldBalanceByAccCode(stri), 3).ToString("0.000");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (rbtCash.Checked == true)
                {
                    ...
                    for (int i = 0; i < this.dgvTransection.Rows.Count - 1; i++)
                    {
                        ...
                        h = Math.Round(Convert.ToDecimal(this.dgvTransection.Rows[i].Cells[2].Value), 0);
                        pv.AccountCode = cha;
                        pv.Dr = h;
                        pv.Cr = 0;
                ...
                if (rbtGold.Checked == true)
                {
                    ...
                    for (int i = 0; i < this.dgvTransection.Rows.Count - 1; i++)
                    {
                        ...
                        h = Math.Round(Convert.ToDecimal(this.dgvTransection.Rows[i].Cells[2].Value), 3);
                        pv.AccountCode = cha;
                        pv.GoldDr = h;
                        pv.GoldCr = 0;
                        pv.DDate = this.dtpDate.Value;
            ...
            finally
            {
                if (con.State == ConnectionState.Open)
                {
                    ...
                    if (rbtGold.Checked == true)
                    {
                        this.txtVoucherNo.Text = vDAL.CreateVNO("GRV");
                        this.ShowLedgerGold();
                    }
                }
            }
        ...
        private void ShowLedger()
        {
            ...
            vouch = vDAL.GetVoucher(this.dtpDate.Value);
            if (vouch == null)
            {
        ...
        private void ShowLedgerGold()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    this.dgvLedger.Rows[i].Cells[3].Value = Math.Round(vouchgld[i].GoldDr, 3).ToString();
                    this.dgvLedger.Rows[i].Cells[4].Value = Math.Round(vouchgld[i].GoldCr, 3).ToString();
        ...
        private void dgvLedger_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (rbtCash.Checked == true)
                this.ShowTransaction(st, 0);
            if (rbtGold.Checked == true)
                this.ShowTransaction(st, 1);
            tabLedger.SelectedTab = tabPage1;
            this.btnEdit.Text = "&Update";
            this.btnSave.Enabled = false;
        }
        private void ShowTransaction(string stn, int a)
        {
            vhs = new List<Voucher>();
            vhs = vDAL.GetVoucherForUpdate(stn);
            if (vhs == null)
            {
                this.dgvTransection.Rows.Clear();
                return;
            ...
            else
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (rbtCash.Checked == true)
                        this.dgvTransection.Rows[i].Cells[2].Value = Math.Round(vhs[i].Cr, 0).ToString();
                    if (rbtGold.Checked == true)
                        this.dgvTransection.Rows[i].Cells[2].Value = Math.Round(vhs[i].GoldCr, 3).ToString();
