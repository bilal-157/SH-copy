...
namespace jewl
{
    public partial class DamageReports : Form
    {
        ...
        private void frmDamageReports_Load(object sender, EventArgs e)
        {
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            {
                frmDamageStock cmp = new frmDamageStock();
                cmp.ShowDialog();
            }
            if (rbtItemWise.Checked == true && cbxGroupItem.SelectedIndex != -1)
            {
                frmDamageItmWise frmitm = new frmDamageItmWise();
                //string selectQuery = "";
            ...
            if (this.rbtDateWise.Checked == true)
            {
                ...
                {
                    frmDamageStock frm = new frmDamageStock();
                    //string selectQuery = "";
                    if (string.IsNullOrEmpty(selectQuery))
                    {
                        //DateTime dt = dtpFrom.Value;

                        selectQuery = "{StockRpt.DamDate}>=Date('" + dtpFromDate.Value.ToShortDateString() + "')";
                    }

                    //if (string.IsNullOrEmpty(selectQuery))
                    selectQuery = selectQuery + "and {StockRpt.DamDate}<=Date('" + dtpToDate.Value.ToShortDateString() + "')";
                ...
                else
                {
                    ...
                    if (string.IsNullOrEmpty(selectQuery))
                    {
                        selectQuery = "{StockRpt.DamDate }=Date('" + dtpDate.Value.ToShortDateString() + "')";
                    }
                    frm.selectQuery = selectQuery;
                    frm.ShowDialog();
                }
        ...
        private void frmDamageReports_Paint(object sender, PaintEventArgs e)
        {
